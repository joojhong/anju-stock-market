/**
 * auth4.js - 안주주식마켓 인증 모듈
 *
 * 잠금 방식: pagehide/visibilitychange 이벤트 기반
 *  - 앱이 숨겨질 때 타임스탬프 기록
 *  - 복귀 시 경과 시간 체크 → LOCK_AFTER ms 이상이면 PIN 재요구
 *  - 페이지 이동은 숨김 시간이 거의 0 → PIN 스킵
 *
 * LOCK_AFTER = 60초 (앱 완전 종료 후 재실행은 항상 이 시간 초과)
 */
(function () {
  'use strict';
  ...
  // 이 시간(ms) 이상 화면에서 사라졌다 돌아오면 PIN 재요구
  const LOCK_AFTER = 60 * 1000; // 60초

  const KEY_TOKEN     = 'anju_token';
  const KEY_EXPIRY    = 'anju_expiry';
  const KEY_ROLE      = 'anju_role';
  const KEY_PIN       = 'anju_pin';
  const KEY_HIDDEN_AT = 'anju_hidden_at'; // 마지막으로 숨겨진 시각
  ...
  // 마지막으로 숨겨진 시각 기록/확인
  const recordHidden  = () => localStorage.setItem(KEY_HIDDEN_AT, String(Date.now()));
  const clearHidden   = () => localStorage.removeItem(KEY_HIDDEN_AT);
  function shouldLock() {
    const t = parseInt(localStorage.getItem(KEY_HIDDEN_AT) || '0', 10);
    if (!t) return false; // 처음 실행 or 이미 처리됨
    return (Date.now() - t) >= LOCK_AFTER;
  }
  ...
  function clearSession() {
    [KEY_TOKEN, KEY_EXPIRY, KEY_ROLE, KEY_PIN, KEY_HIDDEN_AT].forEach(k => localStorage.removeItem(k));
  }
  ...
  function showPinSetupScreen(onDone) {
    ...
    const render = (msg) => {
      showOverlay(`<div class="auth-logo">🔐</div><div class="auth-title">${step==='first'?'PIN 설정':'PIN 확인'}</div><div class="auth-sub">${step==='first'?'4자리 PIN을 설정해요':'같은 PIN을 다시 입력해주세요'}</div>${pinDotsHtml()}<div class="pin-grid" id="pg"></div><div class="auth-msg">${msg||''}</div><button class="pin-key wide" onclick="window.__auth_skip()">나중에 설정할게요</button>`);
      renderKeypad('pg', onKey);
    };
    const onKey = (v) => {
  ...
  function showPinUnlockScreen(onSuccess, onFail) {
    let cur = '', attempts = 0;
    const render = (msg) => {
      showOverlay(`<div class="auth-logo">🔒</div><div class="auth-title">PIN 입력</div><div class="auth-sub">4자리 PIN을 입력해주세요</div>${pinDotsHtml()}<div class="pin-grid" id="pg"></div><div class="auth-msg">${msg||''}</div><button class="pin-key wide" onclick="window.__auth_relogin()">구글 재로그인</button>`);
      renderKeypad('pg', onKey);
  ...
  function attachLockListener() {
    ...
    // 숨겨질 때 타임스탬프 기록
    const onHide = () => recordHidden();

    // pagehide: 페이지 완전히 떠날 때 (앱 종료, 탭 닫기 등)
    window.addEventListener('pagehide', onHide);

    // visibilitychange: 백그라운드/포그라운드 전환
    document.addEventListener('visibilitychange', () => {
      if (document.hidden) {
        onHide();
      } else {
        // 복귀 시 경과 시간 체크
        if (shouldLock() && getPin()) {
          clearHidden();
          showPinUnlockScreen(
            () => { clearHidden(); hideOverlay(); attachLockListener(); },
            () => { clearSession(); startGoogleLogin(); }
          );
        } else {
          clearHidden(); // LOCK_AFTER 미만이면 그냥 클리어
        }
      }
    });
  }
  ...
  function enterApp(role) {
    hideOverlay();
    clearHidden(); // 정상 진입 시 이전 hidden 기록 클리어
