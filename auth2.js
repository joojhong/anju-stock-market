/**
 * auth2.js - 안주주식마켓 인증 모듈
 *
 * 잠금 규칙:
 *  - 앱 재실행(새 세션) → 항상 PIN 요구
 *  - 같은 세션 내 페이지 이동 → PIN 스킵
 *  - 백그라운드 2분 이상 → PIN 재요구
 *
 * 구현: sessionStorage 'anju_alive' 로 세션 감지
 *       PIN 해제되면 sessionStorage 'anju_ok' 설정
 *       앱 재시작 시 sessionStorage 초기화 → PIN 재요구
 */
(function () {
  'use strict';
  ...
  // sessionStorage 키 (탭/앱 닫으면 사라짐)
  const SKEY_ALIVE = 'anju_alive'; // 세션 살아있음 표시
  const SKEY_OK    = 'anju_ok';    // 이 세션에서 PIN 해제됨
  ...
  // 세션 살아있음 표시 (페이지 이동과 앱 재실행을 구분하는 데 사용 안 함 — sessionStorage 자체가 탭 단위)
  const markAlive = () => sessionStorage.setItem(SKEY_ALIVE, '1');
  ...
  function injectStyles() {
    ...
    s.textContent = `
      ...
      #auth-overlay .pin-dot {
        ...
        background:#e0e0e0;transition:background .15s;
      }
  ...
  function showLoginScreen(msg) {
    showOverlay(`
      ...
      <button class="btn-google" onclick="window.__auth_gl()">
  ...
  function showPinSetupScreen(onDone) {
    ...
    function render(msg) {
      showOverlay(`
        ...
        <div class="auth-sub">${step === 'first'
          ? '앱 잠금에 사용할 4자리 PIN을 설정해요'
          : '같은 PIN을 다시 입력해주세요'}</div>
        <div class="pin-dots">
    ...
    function onKey(v) {
      ...
      else if (current.length < 4) current += v;
      updateDots();
  ...
  function showPinUnlockScreen(onSuccess, onFail) {
    ...
    async function tryUnlock() {
      if (await sha256(current) === getPin()) {
        onSuccess();
      } else {
        attempts++;
        current = '';
        attempts >= 5 ? onFail() : render(`틀렸어요. ${5 - attempts}번 더 시도할 수 있어요.`);
      }
    }
    current = ''; render();
  }
  ...
  function enterApp(role) {
    ...
    if (role === 'viewer') {
      const apply = () => {
        ...
        new MutationObserver(applyViewerRestrictions)
  ...
  function proceedToApp(role) {
    if (!getPin()) {
      ...
      // 같은 세션에서 이미 PIN 해제 → 바로 진입 (페이지 이동)
      enterApp(role);
    } else {
      // 새 세션(앱 재실행) → PIN 입력
      showPinUnlockScreen(
